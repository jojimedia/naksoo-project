...
월간 큰손 목록은 부분 응답으로 통째 교체하지 않는다. 같은 후원자 ID의 월 누적 별풍선과 후원 횟수는 이전값과 새 값 중 큰 값을 유지하고, 닉네임·프로필만 최신값으로 갱신한다. 원본 응답에서 잠시 사라진 후원자도 유지하며 스트리머별 상위 100명으로 제한한다. 이 규칙이 없으면 월 총합은 그대로인데 크루 큰손 점수만 100만에서 63만처럼 일시적으로 감소할 수 있다.
...
2026-09-26 기준 격리된 환경에서 백엔드 전체 42개 테스트가 통과했다.
